...
// поэтому браузер не может загружать файлы напрямую.
//
// Решение: этот прокси-сервер принимает изображение от клиента (base64 в JSON),
// загружает его на сервер ВК (сервер-сервер, без CORS) и возвращает attachment.
//
// Запуск: node server/upload-proxy.js
// Требуется Node.js 18+ (fetch + FormData)
 ...
 */
async function uploadToVKServer(uploadUrl, imageBuffer) {
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    console.log(`[upload-proxy] Image: ${imageBuffer.length} bytes`);
    ...
    console.log(`[upload-proxy] Success: ${attachment}`);
